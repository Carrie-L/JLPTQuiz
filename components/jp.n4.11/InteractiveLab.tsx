...
import { Check, X, Wand2, Lightbulb, Ban, AlertTriangle } from 'lucide-react';
...
const InteractiveLab: React.FC<InteractiveLabProps> = ({ config, onComplete }) => {
  ...
  const renderVisualCard = () => (
    <div className={`bg-white rounded-3xl shadow-[0_4px_20px_rgba(0,0,0,0.05)] border-2 border-yellow-200 p-8 mb-8 flex items-center gap-8 relative overflow-hidden ${cardWidthClass}`}>
      <Ban className="absolute -top-2 -right-2 text-red-100 opacity-50 transform rotate-12" size={100} />
      
      <div className="bg-gradient-to-br from-yellow-100 to-orange-100 text-orange-800 w-20 h-20 rounded-2xl flex items-center justify-center text-3xl font-bold border border-yellow-200 shadow-inner relative z-10 shrink-0">
        {config.targetVerb?.group}
      </div>
      <div className="relative z-10 flex-grow">
        <div className="text-4xl font-bold text-stone-800 mb-2 font-['Zen_Maru_Gothic']">
  ...
  const renderInputArea = () => (
      <div className={`flex flex-col items-center ${cardWidthClass}`}>
        ...
        <div className="bg-white/90 backdrop-blur-sm p-10 rounded-[2.5rem] w-full shadow-xl border-2 border-stone-100 relative overflow-hidden">
          {/* Warning Tape */}
          <div className="absolute top-0 left-0 w-full h-3 bg-[repeating-linear-gradient(45deg,#FBBF24,#FBBF24_10px,#000_10px,#000_20px)] opacity-10" />
          ...
          <div className="flex items-center justify-center gap-4 mb-10">
             <input 
                ...
                className={`
                  ...
                  ${feedbackState === 'error' ? 'border-red-300 bg-red-50 animate-shake' : 'border-stone-200 focus:border-yellow-400 focus:bg-white'}
          ...
          {!showAnswer && (
            <button
              ...
              className={`w-full py-5 rounded-2xl font-bold text-xl transition-all shadow-sm
                ${feedbackState === 'success' ? 'bg-emerald-500 text-white' : 'bg-yellow-500 text-white hover:bg-yellow-600 hover:shadow-md'}
        ...
        {config.explanation && feedbackState === 'success' && (
           <p className="text-red-600 font-bold mt-4 bg-red-50 px-4 py-2 rounded-lg border border-red-100 flex items-center gap-2"><AlertTriangle size={16}/> {config.explanation}</p>
        )}
      </div>
  );
            ...
            return (
              <button
                ...
                className={`
                  ...
                  ${feedbackState === 'error' && userAnswer === opt ? 'bg-red-50 border-red-300 text-red-700' : 'bg-white border-stone-100 text-stone-600 hover:border-yellow-400 hover:shadow-md'}
                `}
              >
                {opt}
