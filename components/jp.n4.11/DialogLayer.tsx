...
const DialogLayer: React.FC<DialogLayerProps> = ({ step, canProceed, onNext, onPrev, isLast }) => {
  ...
  return (
    <div className="w-full h-full max-w-5xl mx-auto relative group pt-6">
      <div className="w-full h-full bg-white/95 backdrop-blur-md rounded-[20px] shadow-lg relative flex flex-col p-8 transition-all duration-300 border-b-8 border-yellow-400">
        ...
        <div className="absolute top-0 left-8 z-30 transform -translate-y-1/2">
          <div className={`${nameColorClass} border-2 px-6 py-1.5 rounded-full shadow-sm font-bold tracking-wide text-sm md:text-base flex items-center gap-2`}>
             ...
             {step.speakerColor === 'yellow' && <AlertOctagon size={14} className="text-yellow-600" />}
          </div>
        </div>

        {/* Corner Brackets - Warning Style */}
        <div className="absolute top-4 left-4 w-8 h-8 border-t-[4px] border-l-[4px] border-yellow-400 rounded-tl-sm pointer-events-none" />
        <div className="absolute top-4 right-4 w-8 h-8 border-t-[4px] border-r-[4px] border-yellow-400 rounded-tr-sm pointer-events-none" />
        
        <div className="flex-grow overflow-y-auto mb-4 px-4 custom-scrollbar mt-2">
          <p className="text-xl md:text-2xl font-bold text-slate-800 leading-relaxed font-['Zen_Maru_Gothic']">
            {displayedText}
          </p>
        </div>
        ...
        <div className="flex justify-between items-end px-2">
            ...
            <button
              ...
              className={`
                ...
                ${canProceed 
                  ? 'text-yellow-600 cursor-pointer hover:tracking-widest' 
