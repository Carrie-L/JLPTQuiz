...
import { Book, Heart, Calendar, ArrowDownCircle } from 'lucide-react';
...
const StageLayer: React.FC<StageLayerProps> = ({ step, onComplete }) => {
  ...
  const BackgroundDecorations = () => (
    <div className="absolute inset-0 pointer-events-none overflow-hidden rounded-3xl">
      <Calendar className="absolute top-10 right-10 text-rose-200 opacity-60" size={100} />
      <Heart className="absolute bottom-10 left-10 text-pink-100 opacity-60" size={120} />
    </div>
  );
      ...
      case ViewType.IMAGE:
        return (
          <div className="w-full h-full flex flex-col items-center justify-center p-8 relative">
            <div className="relative w-full max-w-2xl aspect-video bg-[#FFFEF5] rounded-3xl shadow-[0_8px_30px_rgba(0,0,0,0.04)] border-4 border-rose-300 ring-1 ring-rose-200 overflow-hidden flex items-center justify-center group z-10">
               <BackgroundDecorations />
               
               <div className="text-center z-10 relative flex flex-col items-center">
                 <div className="bg-white p-6 rounded-full inline-block mb-4 shadow-sm text-rose-500">
            ...
            {viewContent.imageCaption && (
              <div className="mt-6 bg-white/80 px-6 py-2 rounded-full border border-rose-200 shadow-sm text-rose-700 font-bold tracking-wider text-sm z-10">
                {viewContent.imageCaption}
              </div>
            )}
          </div>
                  ...
                  <div className="space-y-3 bg-white/60 p-6 rounded-2xl border border-stone-100">
                    ...
                    {viewContent.grammarRule?.examples.map((ex, idx) => (
                      <div key={idx} className="flex justify-between items-center p-3 hover:bg-white transition-colors rounded-lg group">
                        <span className="text-xl font-bold text-stone-700 group-hover:text-[#C62828] transition-colors">{ex.jp}</span>
