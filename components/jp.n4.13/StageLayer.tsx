...
import { Book, GitBranch, CloudRain, Umbrella } from 'lucide-react';
...
const StageLayer: React.FC<StageLayerProps> = ({ step, onComplete }) => {
  ...
  const BackgroundDecorations = () => (
    <div className="absolute inset-0 pointer-events-none overflow-hidden rounded-3xl">
      <CloudRain className="absolute top-10 right-10 text-indigo-200 opacity-60" size={100} />
      <GitBranch className="absolute bottom-10 left-10 text-slate-200 opacity-60" size={120} />
    </div>
  );
      ...
      case ViewType.IMAGE:
        return (
          <div className="w-full h-full flex flex-col items-center justify-center p-8 relative">
            <div className="relative w-full max-w-2xl aspect-video bg-[#FFFEF5] rounded-3xl shadow-[0_8px_30px_rgba(0,0,0,0.04)] border-4 border-indigo-200 ring-1 ring-indigo-100 overflow-hidden flex items-center justify-center group z-10">
               <BackgroundDecorations />
               
               <div className="text-center z-10 relative flex flex-col items-center">
                 <div className="bg-white p-6 rounded-full inline-block mb-4 shadow-sm text-indigo-500">
                    <Umbrella size={48} className="opacity-80" />
                 </div>
                 <p className="text-stone-500 font-medium px-8 font-serif text-lg">{viewContent.imagePrompt}</p>
               </div>
            </div>
            {viewContent.imageCaption && (
              <div className="mt-6 bg-white/80 px-6 py-2 rounded-full border border-indigo-100 shadow-sm text-indigo-700 font-bold tracking-wider text-sm z-10">
                {viewContent.imageCaption}
              </div>
            )}
          </div>
      ...
      case ViewType.GRAMMAR_EXPLAIN:
        return (
          <div className="w-full max-w-4xl bg-[#FFFEF5] rounded-3xl shadow-xl overflow-hidden border border-stone-200 relative z-10">
            {/* Header - Indigo for Conditional */}
            <div className="bg-[#5C6BC0] p-6 flex items-center gap-3 relative overflow-hidden">
            ...
            <div className="p-8 relative">
               ...
               <div className="relative z-10">
                  <div className="space-y-4 mb-8">
                    {viewContent.grammarRule?.content.map((rule, idx) => (
                      <div key={idx} className="flex items-start gap-4 p-5 bg-white/80 backdrop-blur-sm rounded-xl border border-stone-100 shadow-sm">
                        <div className="w-8 h-8 rounded-full bg-[#9FA8DA] text-indigo-900 flex items-center justify-center font-bold text-base shrink-0">
                          {idx + 1}
                        </div>
                  ...
                  <div className="space-y-3 bg-white/60 p-6 rounded-2xl border border-stone-100">
                    <h3 className="text-sm font-bold text-[#5C6BC0] uppercase tracking-widest mb-2 border-b border-[#5C6BC0]/20 pb-2">例句 (Examples)</h3>
                    {viewContent.grammarRule?.examples.map((ex, idx) => (
                      <div key={idx} className="flex justify-between items-center p-3 hover:bg-white transition-colors rounded-lg group">
                        <span className="text-xl font-bold text-stone-700 group-hover:text-[#3949AB] transition-colors">{ex.jp}</span>
