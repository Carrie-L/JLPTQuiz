...
import { Check, X, Wand2, Lightbulb, Megaphone } from 'lucide-react';
...
const InteractiveLab: React.FC<InteractiveLabProps> = ({ config, onComplete }) => {
  ...
  const renderVisualCard = () => (
    <div className={`bg-white rounded-3xl shadow-[0_4px_20px_rgba(0,0,0,0.05)] border border-stone-100 p-8 mb-8 flex items-center gap-8 relative overflow-hidden ${cardWidthClass}`}>
      <Megaphone className="absolute -top-2 -right-2 text-red-100 opacity-50 transform rotate-12" size={100} />
      
      <div className="bg-gradient-to-br from-red-50 to-orange-50 text-red-700 w-20 h-20 rounded-2xl flex items-center justify-center text-3xl font-bold border border-red-100 shadow-inner relative z-10 shrink-0">
          ...
          <div className="flex items-center justify-center gap-4 mb-10">
             <input 
                ...
                className={`
                  ...
                  ${feedbackState === 'error' ? 'border-red-300 bg-red-50 animate-shake' : 'border-stone-200 focus:border-red-400 focus:bg-white'}
          ...
          {!showAnswer && (
            <button
              ...
              className={`w-full py-5 rounded-2xl font-bold text-xl transition-all shadow-sm
                ${feedbackState === 'success' ? 'bg-emerald-500 text-white' : 'bg-red-500 text-white hover:bg-red-600 hover:shadow-md'}
            ...
            return (
              <button
                ...
                className={`
                  ...
                  ${feedbackState === 'error' && userAnswer === opt ? 'bg-red-50 border-red-300 text-red-700' : 'bg-white border-stone-100 text-stone-600 hover:border-red-300 hover:shadow-md'}
                `}
              >
                {opt}
