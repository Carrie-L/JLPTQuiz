...
const DialogLayer: React.FC<DialogLayerProps> = ({ step, canProceed, onNext, onPrev, isLast }) => {
  ...
  const nameColorClass = step.speakerColor === 'pink' 
    ...
    : step.speakerColor === 'red'
    ? "bg-[#FFEBEE] text-[#C62828] border-[#FFCDD2]"
    : "bg-[#E0F2F1] text-[#00695C] border-[#B2DFDB]"; // Teal default

  return (
    <div className="w-full h-full max-w-5xl mx-auto relative group pt-6">
      <div className="w-full h-full bg-white/95 backdrop-blur-md rounded-[20px] shadow-lg relative flex flex-col p-8 transition-all duration-300 border-t-4 border-red-500/20">
        ...
        <div className="absolute top-0 left-8 z-30 transform -translate-y-1/2">
          <div className={`${nameColorClass} border-2 px-6 py-1.5 rounded-full shadow-sm font-bold tracking-wide text-sm md:text-base flex items-center gap-2`}>
             ...
             {step.speakerColor === 'red' && <AlertTriangle size={14} className="animate-pulse" />}
          </div>
        </div>

        {/* Corner Brackets - Red for this module */}
        <div className="absolute top-4 left-4 w-6 h-6 border-t-[3px] border-l-[3px] border-red-400 rounded-tl-lg pointer-events-none" />
        <div className="absolute top-4 right-4 w-6 h-6 border-t-[3px] border-r-[3px] border-red-400 rounded-tr-lg pointer-events-none" />
        <div className="absolute bottom-4 left-4 w-6 h-6 border-b-[3px] border-l-[3px] border-red-400 rounded-bl-lg pointer-events-none" />
        <div className="absolute bottom-4 right-4 w-6 h-6 border-b-[3px] border-r-[3px] border-red-400 rounded-br-lg pointer-events-none" />
        ...
        <div className="flex justify-between items-end px-2">
            ...
            <button
              ...
              className={`
                ...
                ${canProceed 
                  ? 'text-red-600 cursor-pointer hover:tracking-widest' 
