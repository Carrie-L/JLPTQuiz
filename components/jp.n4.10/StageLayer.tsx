...
import { Book, Megaphone, AlertTriangle, Siren } from 'lucide-react';
...
const StageLayer: React.FC<StageLayerProps> = ({ step, onComplete }) => {
  ...
  const BackgroundDecorations = () => (
    <div className="absolute inset-0 pointer-events-none overflow-hidden rounded-3xl">
      <Siren className="absolute top-10 right-10 text-red-100 opacity-60" size={80} />
      <Megaphone className="absolute bottom-10 left-10 text-orange-100 opacity-60" size={100} />
    </div>
  );
      ...
      case ViewType.IMAGE:
        return (
          <div className="w-full h-full flex flex-col items-center justify-center p-8 relative">
            <div className="relative w-full max-w-2xl aspect-video bg-[#FFFEF5] rounded-3xl shadow-[0_8px_30px_rgba(0,0,0,0.04)] border-4 border-red-100 ring-1 ring-red-200 overflow-hidden flex items-center justify-center group z-10">
               <BackgroundDecorations />
               
               <div className="text-center z-10 relative flex flex-col items-center">
                 <div className="bg-white p-6 rounded-full inline-block mb-4 shadow-sm text-red-600 animate-bounce">
            ...
            {viewContent.imageCaption && (
              <div className="mt-6 bg-red-500/90 text-white px-8 py-2 rounded-full shadow-md font-bold tracking-wider text-sm z-10 border border-red-600">
                {viewContent.imageCaption}
              </div>
            )}
          </div>
      ...
      case ViewType.GRAMMAR_EXPLAIN:
        return (
          <div className="w-full max-w-4xl bg-[#FFFEF5] rounded-3xl shadow-xl overflow-hidden border border-stone-200 relative z-10">
            {/* Header - Red for Imperative */}
            <div className="bg-[#E53935] p-6 flex items-center gap-3 relative overflow-hidden">
            ...
            <div className="p-8 relative">
               ...
               <div className="relative z-10">
                  <div className="space-y-4 mb-8">
                    {viewContent.grammarRule?.content.map((rule, idx) => (
                      <div key={idx} className="flex items-start gap-4 p-5 bg-white/80 backdrop-blur-sm rounded-xl border border-stone-100 shadow-sm">
                        <div className="w-8 h-8 rounded-full bg-[#EF9A9A] text-white flex items-center justify-center font-bold text-base shrink-0">
                          {idx + 1}
                        </div>
                  ...
                  <div className="space-y-3 bg-white/60 p-6 rounded-2xl border border-stone-100">
                    <h3 className="text-sm font-bold text-[#E53935] uppercase tracking-widest mb-2 border-b border-[#E53935]/20 pb-2">例句 (Examples)</h3>
                    {viewContent.grammarRule?.examples.map((ex, idx) => (
                      <div key={idx} className="flex justify-between items-center p-3 hover:bg-white transition-colors rounded-lg group">
                        <span className="text-xl font-bold text-stone-700 group-hover:text-[#B71C1C] transition-colors">{ex.jp}</span>
