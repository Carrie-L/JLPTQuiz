...
const VictoryScreen: React.FC<VictoryScreenProps> = ({ onExit }) => {
  return (
    <div className="w-full h-screen bg-[#FFFDF5] flex flex-col items-center justify-center p-8 text-center relative overflow-hidden">
      ...
      <div className="absolute top-10 left-10 w-4 h-4 bg-violet-400 rounded-full animate-bounce" style={{ animationDelay: '0.1s' }} />
      <div className="absolute top-20 right-20 w-3 h-3 bg-indigo-400 rotate-45 animate-pulse" />

      <div className="bg-white p-12 rounded-[3rem] shadow-2xl border-4 border-violet-100 max-w-lg w-full flex flex-col items-center z-10">
        ...
        <p className="text-slate-500 mb-8 font-medium">使役形 (Causative) 已经掌握了。</p>
        ...
        <div className="bg-slate-50 w-full rounded-2xl p-6 mb-8 text-left">
           ...
           <ul className="space-y-2 text-slate-700 font-medium">
              <li className="flex items-center gap-2"><div className="w-2 h-2 bg-violet-500 rounded-full" /> 一类: u → a + seru</li>
              <li className="flex items-center gap-2"><div className="w-2 h-2 bg-violet-500 rounded-full" /> 二类: 去掉 ru + saseru</li>
              <li className="flex items-center gap-2"><div className="w-2 h-2 bg-indigo-500 rounded-full" /> 不规则: Saseru / Kosaseru</li>
           </ul>
        </div>
