...
const GameEngine: React.FC<GameEngineProps> = ({ onExit }) => {
  ...
  return (
    <div className="relative w-full h-screen bg-[#FFFDF5] overflow-hidden flex flex-col font-['Zen_Maru_Gothic']">
      {/* Background Decor - Amber/Dawn Theme for Effort/Hardship */}
      <div className="absolute inset-0 pointer-events-none">
         <div className="absolute -top-20 left-1/2 w-[800px] h-[800px] bg-amber-100/40 rounded-full blur-[100px] transform -translate-x-1/2" />
         <div className="absolute bottom-[-100px] right-[-100px] w-[600px] h-[600px] bg-orange-50/60 rounded-full blur-[120px]" />
      </div>

      <button 
        onClick={onExit}
        className="absolute top-4 left-4 z-50 flex items-center gap-2 px-4 py-2 bg-white/90 backdrop-blur-sm border-2 border-amber-100 rounded-full text-amber-700 font-bold hover:bg-amber-50 hover:border-amber-200 hover:shadow-md transition-all text-sm group"
      >
