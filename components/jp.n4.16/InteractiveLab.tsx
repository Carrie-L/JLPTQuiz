...
import { Check, X, Wand2, Lightbulb, MessageCircle } from 'lucide-react';
...
const InteractiveLab: React.FC<InteractiveLabProps> = ({ config, onComplete }) => {
  ...
  const renderVisualCard = () => (
    <div className={`bg-white rounded-3xl shadow-[0_4px_20px_rgba(0,0,0,0.05)] border border-stone-100 p-8 mb-8 flex items-center gap-8 relative overflow-hidden ${cardWidthClass}`}>
      <MessageCircle className="absolute -top-2 -right-2 text-teal-100 opacity-50 transform rotate-12" size={100} />
      
      <div className="bg-gradient-to-br from-teal-50 to-green-50 text-teal-700 w-20 h-20 rounded-2xl flex items-center justify-center text-3xl font-bold border border-teal-100 shadow-inner relative z-10 shrink-0">
      ...
      <div className="relative z-10 flex-grow">
        ...
        <div className="text-sm text-stone-300 mt-2 uppercase tracking-wide font-bold">
            {config.targetVerb?.group === 3 ? "名词/不规则" : config.targetVerb?.group === 2 ? "二类动词/形容词" : "一类动词"}
        </div>
      </div>
    </div>
  );
          ...
          {!showAnswer && (
            <button
              ...
              className={`w-full py-5 rounded-2xl font-bold text-xl transition-all shadow-sm
                ${feedbackState === 'success' ? 'bg-emerald-500 text-white' : 'bg-teal-500 text-white hover:bg-teal-600 hover:shadow-md'}
