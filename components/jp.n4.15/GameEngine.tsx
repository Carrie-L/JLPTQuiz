...
const GameEngine: React.FC<GameEngineProps> = ({ onExit }) => {
  ...
  return (
    <div className="relative w-full h-screen bg-[#F1F5F9] overflow-hidden flex flex-col font-['Zen_Maru_Gothic']">
      {/* Background Decor - Slate/Stone Theme for Immutable Facts */}
      <div className="absolute inset-0 pointer-events-none">
         <div className="absolute -top-20 right-0 w-[600px] h-[600px] bg-slate-200/60 rounded-full blur-[100px]" />
         <div className="absolute bottom-[-100px] left-[-100px] w-[600px] h-[600px] bg-gray-200/60 rounded-full blur-[120px]" />
         {/* Geometric Compass/Clock Concept */}
         <div className="absolute top-20 left-20 w-32 h-32 border-4 border-slate-300/30 rounded-full animate-[spin_10s_linear_infinite]" />
         <div className="absolute bottom-40 right-40 w-48 h-48 border-2 border-slate-300/20 rounded-full animate-[spin_15s_linear_infinite_reverse]" />
      </div>

      <button 
        onClick={onExit}
        className="absolute top-4 left-4 z-50 flex items-center gap-2 px-4 py-2 bg-white/90 backdrop-blur-sm border-2 border-slate-200 rounded-full text-slate-700 font-bold hover:bg-slate-50 hover:border-slate-300 hover:shadow-md transition-all text-sm group"
      >
