...
const GameEngine: React.FC<GameEngineProps> = ({ onExit }) => {
  ...
  return (
    <div className="relative w-full h-screen bg-[#FFFDF5] overflow-hidden flex flex-col font-['Zen_Maru_Gothic']">
      {/* Background Decor - Lime Theme */}
      <div className="absolute inset-0 pointer-events-none">
         <div className="absolute -top-20 left-20 w-[600px] h-[600px] bg-lime-100/40 rounded-full blur-[100px]" />
         <div className="absolute bottom-0 right-0 w-[500px] h-[500px] bg-green-50/60 rounded-full blur-[120px]" />
      </div>

      <button 
        onClick={onExit}
        className="absolute top-4 left-4 z-50 flex items-center gap-2 px-4 py-2 bg-white/90 backdrop-blur-sm border-2 border-lime-100 rounded-full text-lime-700 font-bold hover:bg-lime-50 hover:border-lime-200 hover:shadow-md transition-all text-sm group"
      >
